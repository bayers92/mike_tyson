class HomepageInfosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @homepage_info.save
          format.html { redirect_to edit_homepage_info_path(current_user.homepage_info), notice: 'Homepage Info was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @homepage_info.save
          format.html { redirect_to :back, notice: 'Homepage Info was successfully updated.' }
          format.json { render :show, status: :created, location: @homepage_info }
      else
        format.html { render :new }
        format.json { render json: @homepage_info.errors, status: :unprocessable_entity }
